...
A **Shade Agent** is an application built using the Shade Agent Framework. See [What can you build?](./what-can-you-build) for some examples.
...
The agent contract can sign transactions for **most chains** via NEAR's [Chain Signatures](../../../chain-abstraction/chain-signatures) service, which is accessed by making cross-contract calls to the **MPC contract**. All assets and state of the Shade Agent should be tied to the agent contract, not the agent's account, since agent keys are ephemeral.
