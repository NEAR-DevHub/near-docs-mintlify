...
The repo also contains an **agent contract**. We won't review the agent contract as it's the same as the reference implementation [featured here](../../reference/agent-contract), but we encourage you to review the reference implementation after this section.
...
<Github fname="index.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/index.ts#L36-L46"
    start="36" end="46" />
...
<Github fname="index.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/index.ts#L51-L54"
    start="51" end="54" />
...
<Github fname="index.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/index.ts#L56-L72"
    start="56" end="72" />

Since registrations expire (every 7 days by default), an interval is set to **re-register** the agent every 6 days.

<Github fname="index.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/index.ts#L75-L85"
    start="75" end="85" />
...
In this example, the agent is calling the `request_signature` function on the agent contract. This function takes a transaction payload for nearly any blockchain and requests a signature via [Chain Signatures](../../../../chain-abstraction/chain-signatures/implementation). Here, we're signing a transaction to call an Ethereum contract to update the stored price of ETH. First, we retrieve the price of ETH (in this example, the function queries two different APIs and calculates the average).

<Github fname="transaction.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/routes/transaction.ts#L25"
    start="25" end="25" />
...
<Github fname="transaction.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/routes/transaction.ts#L68-L84"
    start="68" end="84" />
...
<Github fname="transaction.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/routes/transaction.ts#L37-L44"
    start="37" end="44" />
...
<Github fname="transaction.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/routes/transaction.ts#L47-L54"
    start="47" end="54" />
...
<Github fname="ethereum.ts" language="TypeScript"
    url="https://github.com/NearDeFi/shade-agent-template/tree/main/src/utils/ethereum.ts#L42-L51"
    start="42" end="51" />
...
Implementation details differ slightly from chain to chain; as such, we recommend you review our [chain signature docs](../../../../chain-abstraction/chain-signatures/implementation). Note that step 3 of requesting a signature is different; we use the `agent.call()` method from `shade-agent-js` instead.
...
As you move into production, it's recommended to implement on-chain guardrails in your agent contract to prevent malicious actions even if the TEE is compromised. You can read more about guardrails [here](../../concepts/terminology#on-chain-guardrails).

---

## Next Steps

Now that you've explored the basics of Shade Agents, we recommend diving deeper into the [framework overview](../../concepts/framework-overview) to understand the core components for building production-ready Shade Agents.
