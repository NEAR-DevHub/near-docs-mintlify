---
title: Shade Agents
sidebarTitle: What are Shade Agents?
...
<Info>
Shade Agents power [Agentic Protocols](../concepts/what-can-you-build#agentic-protocols): a new type of decentralized application designed to be autonomous, proactive, and intelligent.
</Info>
...
- The [measurements](../concepts/terminology#measurements) of the code running in the TEE (which uniquely identifies the code and platform it's running on).

If the attestation is valid, the measurements fit the approved measurements in the contract, and it's running on a certain machine (identified by a [PPID](../concepts/terminology#ppid)), then the agent's account is registered inside the agent contract.
...
<Tip>
`request_signature` is only an example of an agent-gated function. You can implement any function you want on the agent contract. It's recommended to implement [on-chain guardrails](../concepts/terminology#on-chain-guardrails) to prevent unauthorized actions even in the case of a compromised TEE.
</Tip>
...
Now that you've learned what Shade Agents are and why they're powerful, head to the [quickstart](./quickstart/deploying) to deploy your first agent and see how to build them.
