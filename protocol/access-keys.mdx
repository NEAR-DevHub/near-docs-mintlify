...
Learn more about the benefits of using multiple keys in our blog post: [Benefits of Function-Call Keys](https://docs.near.org/blog/benefits-of-multiple-keys)
...
3. `allowance` (Optional): The **amount of NEAR** allowed to be spent on [gas](/protocol/gas). If omitted, the key can consume **unlimited** gas

`Function Call Keys` are meant to be shared with applications, so third-parties can make contract calls in your name. This is useful in [multiple scenarios as we will see below](https://docs.near.org/blog/benefits-of-multiple-keys).
