...
NEAR consists roughly of a blockchain layer and a [runtime layer](/protocol/network/runtime).
...
[Transactions](/protocol/transactions) and receipts are a fundamental concept in Near Protocol. Transactions represent actions requested by the
...
Similar to Ethereum, Near Protocol is an [account-based system](/protocol/account-model). Which means that each blockchain user is roughly
associated with one or several accounts (there are exceptions though, when users share an account and are separated
through the [access keys](/protocol/access-keys)).
...
Even though [tokens](/protocol/network/tokens) is a fundamental concept of the blockchain, it is neatly encapsulated
inside the runtime layer together with the [gas](/protocol/gas), [fees](/protocol/gas#understanding-gas-fees), and rewards.
...
responsible for maintaining the integrity of the Near Protocol. These participants called [Validators](/protocol/network/validators) are associated with the
...
- [Fees and rewards](/protocol/gas) -- fees and rewards are neatly encapsulated in the runtime layer. The blockchain layer, however,
