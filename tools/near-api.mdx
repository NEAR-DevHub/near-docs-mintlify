...
You can send multiple [actions](/protocol/transaction-anatomy#actions) in a batch to a **single** receiver. If one action fails then the entire batch of actions will be reverted.
...
A [Full Access key](/protocol/access-keys#full-access-keys) grants complete control over the account.
...
A [Function Call access key](/protocol/access-keys#function-call-keys) is designed specifically to sign transactions that include only [`functionCall` actions](/protocol/transaction-anatomy#actions) to a specific contract.
