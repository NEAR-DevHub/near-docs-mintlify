...
Firstly, we'll need the compiled auction contract WASM file. You can get this by running the following command in [03-bid-with-fts](https://github.com/near-examples/auctions-tutorial/tree/main/contract-rs/03-bid-with-fts) of `contract-rs`
...
In this fork, we have also removed the option to add an access key to the contract account since, as discussed [earlier](./1.3-deploy#locking-the-contract), we want auctions to be locked.
