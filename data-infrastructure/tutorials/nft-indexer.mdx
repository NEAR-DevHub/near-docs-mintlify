...
The indexer is watching for `nft_mint` [Events](https://github.com/near/NEPs/tree/master/neps/nep-0297) and prints some relevant data:
...
NEAR Protocol had introduced a nice feature [Events](https://github.com/near/NEPs/tree/master/neps/nep-0297). The Events allow a contract developer to add standardized logs to the `ExecutionOutcomes` thus allowing themselves or other developers to read those logs in more convenient manner via API or indexers.
...
Please, ensure you've the credentials set up as described on the [Credentials](./running-near-lake/credentials) page. Otherwise you won't be able to get the code working.
...
- [Events](https://github.com/near/NEPs/tree/master/neps/nep-0297) built on top of the logs
...
Also, we need to define an interface to catch the Events. Let's copy the interface definition from the [Events Nomicon page](https://github.com/near/NEPs/tree/master/neps/nep-0297#events) and paste it before the `handleStreamerMessage` function.
...
Now we want to deal only with those `ExecutionOutcomes` that contain logs of Events format. Such logs start with `EVENT_JSON:` according to the [Events docs](https://github.com/near/NEPs/tree/master/neps/nep-0297#events).
...
Having troubles running the indexer? Please, check you haven't skipped the [Credentials](./running-near-lake/credentials) part :)
...
Mintbase turn, we hope [Nate](https://twitter.com/nategeier) and his team have migrated to [NEAR Lake Framework](./near-lake-framework) already, saying "Hi!" and crafting the link:
...
- You've learnt about [Events](https://github.com/near/NEPs/tree/master/neps/nep-0297)
...
The material from this tutorial can be extrapolated for literally any event that follows the [Events format](https://github.com/near/NEPs/tree/master/neps/nep-0297)
