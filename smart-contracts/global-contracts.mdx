...
With [NEP-0591](https://github.com/near/NEPs/blob/master/neps/nep-0591), which introduces Global Contracts, NEAR provides a highly strategic alternative that [solves this elegantly](#solution).
...
- **_Are you working in a local environment?_**
  ...
  If you're just testing or building a prototype, [regular deployments](release/deploy) are simpler and more flexible. There's no need to burn tokens or register global references — just deploy and iterate.
...
Global contracts can be deployed using [`NEAR CLI`](../tutorials/examples/global-contracts#deployment) or by code using [NEAR APIs](../tools/near-api#deploy-a-global-contract). Check [this tutorial](../tutorials/examples/global-contracts) to learn how to deploy and use Global Contracts using CLI, JavaScript, or Rust.
